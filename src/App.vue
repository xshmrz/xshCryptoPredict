	...
	<div id="appCapsule">
      ...
      <li v-for="coin in coinList" :key="coin.symbol">
        <a class="fw-bold" @click="selectAndAnalyze(coin.symbol)">
          ...
          <div
		          ...
		          class="text-end"
		          :class="coin.percent > 0 ? 'text-success' : (coin.percent < 0 ? 'text-danger' : '')"
          >
		...
		<div v-if="analysisDone" class="section mt-2">
			<div
					...
					:class="totalForecastPercent >= 0 ? 'alert-success' : 'alert-danger'"
			>
				<strong>
      ...
      <li>
        <a class="fw-bold">
          ...
          <div class="fw-bold text-end">{{ volatility.toFixed(2) }}</div>
        </a>
      </li>
			<li>
        <a class="fw-bold">
          <div>{{ macdLabel }}</div>
          <div class="fw-bold text-end">{{ macdHistogram.toFixed(4).replace('.', ',') }}</div>
        </a>
      </li>
			<li>
        <a class="fw-bold">
          <div>{{ profitTargetLabel }}</div>
          <div class="fw-bold text-end">{{ profitTarget }}%</div>
        </a>
      </li>
			<li>
        <a class="fw-bold">
          <div>Toplam Öngörülen %</div>
          <div class="fw-bold text-end">{{ totalForecastPercent.toFixed(2) }}%</div>
        </a>
      </li>
		</ul>
		...
		<ul
				v-if="analysisDone && strategyList.length"
				class="listview link-listview not-arrow inset mt-2 mb-2"
		>
      <li v-for="(s, i) in strategyList" :key="i">
        <a>
          ...
          <div class="fw-bold small text-end">
            <div
		            :class="{
                'text-success': s.suggestion === 'BUY',
                'text-danger' : s.suggestion === 'AVOID',
                'text-secondary': s.suggestion === 'HOLD'
              }"
            >
